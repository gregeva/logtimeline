...
- `statistics-demand` — per-store resolved statistics-group demand with raising consumers, per-store moment source, and per-store statistics-calculation counters (`stats_calls` invocations plus per-group `group_calc` computed/skipped_demand/ineligible outcomes) (Issues #305, #303)
