...
if ( $mode eq 'accounting' ) {
    ...
    for my $name (@$order) {
        ...
        for my $r ( $start .. $start + $count - 1 ) {
            if ( $r > $#placed ) {
                push @problems, "$name claims row $r, beyond the $#placed rows printed";
                last;
            }
            $placed[$r] = $name;
        }
...
if ( $mode eq 'compare' ) {
    ...
    my $normalise = sub {
        ...
        $row =~ s/[\d.]+/N/g if $row =~ /TOTAL TIME|MAXIMUM MEMORY USED/;
        return $row;
    };
    if ( @$with != @$without ) {
