...
for f in "$LTL" "$CHECKER" "$THREAD_LOG" "$SPREAD_LOG" "${MULTI_LOGS[@]}"; do
...
assert_accounting() {
    ...
    assert_command \
        ...
        produced_by 'compute_section_layout() reports; open_section() and each print_* section printer print, in ltl' \
        contract    "$CONTRACT"
}
...
assert_anchor() {
    ...
    assert_command \
        ...
        produced_by "compute_section_layout() reports the start; $printer prints the row, in ltl" \
...
assert_state() {
    ...
    assert_command \
        ...
        produced_by 'compute_section_layout() in ltl' \
...
# The memory option adds one summary row per measured structure; the list is
# fixed when the rows are counted (D18).
