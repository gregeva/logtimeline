...
- `features/396-windchill-method-server-log4j-format.md` (#396) — adds the `mt17` Windchill Method Server log4j entry: the log4j `%d %-5p [%t] %c %x - %m` layout, local time, occurrences-only; filename family `<Service>-<yyMMddHHmm>-<pid>-log4j.log` with the daily-roll `date_n` index form. Carries D54–D56.
