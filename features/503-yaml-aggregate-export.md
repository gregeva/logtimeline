...
- Phase: **implementation planning** — every decision D1–D22 locked on 2026-09-02; the code and tooling sites the work touches are mapped into § *Implementation plan*, which awaits the architect's walkthrough. Acceptance criteria, the two `-V` section contracts and the prototype charter remain under review. No branch, no code. Produced from a code audit of `release/0.18.0`; every statement of the form "the code does X" was verified on that tree on 2026-09-02 and cites the producing sub and a snippet to grep for.
...
## Implementation plan (2026-09-02 — awaiting the architect's walkthrough)

Four drops, each proved by its own assertions before the next starts. Two branches: drop 1 alone (the line accounting; no YAML; releases the counters #454 needs), then drops 2, 3 and 4 together (the file and everything that exists only for it). Each branch is one PR and one completion gate. Code is cited by sub and a snippet to grep for.
...
- **Gate.** The section contract locked (§ below); P1 before/after on a case with active filters; the full suite.
...
- The `-V aggregate-export` section contract, to lock before drop 4 (§ Observability).
- The heatmap comparison scenario: the bucket store and the heatmap resolve model and bins-per-decade separately (`bpd_for_surface('bucket-stats')` vs `bpd_for_surface('heatmap')`), so the scenario pins both to `bin` and the harness reads each surface's bins-per-decade from `-V percentile-algorithm`; whether both can be made equal for the scenario is settled at harness design.
...
- `tests/baseline/README.md` states the `full` and `all` tiers as 45 and 63 tests; `run-benchmark.sh`'s header states 7 × 10 = 70, and its `quick` description names `twx-unique-errors-standard` while `should_run_test()` matches `single-day-application-log-standard`.
- `tests/HARNESS-DESIGN.md` § Reserved section names lists `filter-summary` under "reserved by sub-issues, not yet implemented" for #229/#230; it moves to *Implemented* with this doc as owner in the same change as the section.
