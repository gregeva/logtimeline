...
- Phase: **implementation, drop 1 delivered on its branch** — every decision D1–D22 locked on 2026-09-02; the execution plan (two branches: drop 1 alone, then drops 2–4) approved by the architect the same day, after every cited sub, variable, fixture and tooling site was re-verified on `release/0.18.0` at 159e886. Drop 1 (the line accounting and `-V filter-summary`) is committed and pushed on branch `503-yaml-aggregate-export` (§ *Drop 1 record*); drops 2–4 follow on the same branch, one commit each, one PR at the end. The `-V filter-summary` contract is locked; the `-V aggregate-export` contract is still to lock before drop 4. Produced from a code audit of `release/0.18.0`; every statement of the form "the code does X" was verified on that tree on 2026-09-02 and cites the producing sub and a snippet to grep for.
