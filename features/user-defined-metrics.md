...
| Default pattern for counting configs | Token capture, form-1 only: `\bname\s*[=:]\s*([^\s,;"'\])]+)` — `]` and `)` excluded from the token | The numeric default patterns can never match `userId=abc123`; the reversed form-2 pattern is too greedy for arbitrary tokens; excluding `]`/`)` keeps bracket-delimited fields clean (ThingWorx `[U: Administrator]` yields `Administrator`, not `Administrator]`) |
