...
- **`fields: user=N/L session=N/L thread=N/L`** (#444 D8), after the variant-group blocks, for the entry the file bound: the identity fields it declares, each as populated lines over the sampled lines the member's pattern matched (populated: defined, non-empty, not a bare `-`); `-` for a field the member does not declare. The same count earns `field_populated` (0.5, one weight table) per populated field in `candidates:`, so a column that has data ranks above the same shape whose column is empty.
...
- The prototype's sampler records file size and observed average line length per part. These were expected to feed a percentage-based progress indicator; they do not — the shipped indicators read the production handle's byte position against a `stat` size (see D53).
...
6. A code comment at the pass naming #397 (percentage-based progress) as a future consumer of the size and average-line-length observations. Not delivered, and correctly so: #397 and #446 (overall percentage across all files) both read the production handle's byte position against a `stat` size, so the sampling pass has no progress consumer to name.
...
Delivered in 0.17.0 (PR #421). Scan subs are compiled when detection elects a format, not eagerly at startup. The investigation that produced these decisions — the problem measurement, the three-compile-source audit, the staged implementation plan and its per-stage results — is recorded on [issue #413](https://github.com/gregeva/logtimeline/issues/413).
...
### Traps and contracts for future work in this area
...
- **F11a — which startup gate touches the record lexicals, and what a transform may leave in them.** Gate 2, the extraction-parity loop, executes every entry's real extraction closure — transforms included — over that entry's declared samples, against the shared record lexicals, before the first line of input is read. Gate 5's startup half does not: it classifies each sample through the interpreted classifier `format_classify_interpreted()` and runs no extraction. The consequence is a contract on what a transform may assign: **a transform may not leave a shared record lexical holding a value Perl represents as a double.** A scalar body grows and never shrinks, every copy inherits it, and one entry's sample is enough to make every duration the statistics model retains afterwards, for every format and every file, carry an unused slot. A computed value is assigned as a string (concatenate the empty string, the form `duration_from_unit_token` uses) or as an integer (free: `$fd->{duration_sum} += $duration` in `read_and_process_logs()` reads every duration numerically ahead of every retention site). `format_record_reset()` restores the values and cannot restore the shape. Full record, with the measurements, the regression assertion and the prototype plan: `features/528-record-lexical-retained-representation.md`.
- **F12 — the D62 RSS instrument distorts the D64 timing row if both boundaries coincide.** With `TIMING detect/scan_sub_compile` bracketing the whole of `compile_format_scan_sub()`, the mixed fixture reported 0.032 s for 3 compiles (~10.7 ms each) against a codegen+validation cost of 3.4 ms each (codegen 3.1 ms, per-compile validation 0.3 ms — the latter measured, and cheap). The gap is `get_memory_usage()`, which shells out to `ps` on macOS and is called twice per compile when D62 measurement is armed. **The D64 timing boundary must exclude the D62 RSS readings** — otherwise the timing row reports the cost of measuring memory rather than the cost of compiling, and only on armed runs, which is exactly where a reader would compare the two. It would also make the row's value depend on whether measurement was armed. **As delivered:** one timer pair spans the whole compile (source build, `eval`, per-compile validation) with the RSS readings hoisted outside it on both sides, so the row reports the same value armed or not. Measured split per compile: source build 0.2 ms, `eval` 3.2 ms, validation 0.3 ms; the mixed fixture's three compiles report 0.012 s either way, against 0.032 s under the naive boundary.
...
- **F14 — the D53 "input that cannot be repositioned" fallback is not reachable through non-seekable input.** `ltl` has no stdin input mode (`-` is treated as a filename), and a named pipe is rejected at input validation — `Error: unable to open any files`, identically on the pre-branch tree and this one, before the detection path is reached. So the held-line window fallback is reachable only via the hidden `--detection-window=N` override, which is how it is exercised instead (window engaged at 50, all 300 lines matched, 1 compile, byte parity with baseline). The dual process's fallback arm therefore guards `sample_file_for_detection()` returning undef for a path that is not a plain file — a state today's input validation makes unreachable, and which would become live if `ltl` ever accepts a stream. Not a defect; recorded so the next reader does not go looking for a case that cannot be built.
...
- **F16 — the scan sub is the producer of the per-message statistics store, and its per-line allocation pattern sets that store's heap layout.** #415's locality ladder and bisect (`tests/profile/results/415-statsdrift-new/analysis.md`) attribute a +15% statistics-phase cost to `bd8a972` (S5): same keys, same entry shape, but the entries — one hash per message, allocated when the message is first seen between the scan sub's per-line temporaries — land less densely, and every later traversal (statistics, sorting, memory measurement) pays for the layout. Not a scan-sub defect, and not fixable in the scan sub: #426 replaces the one-hash-per-message store with a compact ordinal-indexed representation, and the scan sub's entry-creation contract is rewritten against that store when #426 lands. Until then, any change to what the scan sub allocates or frees per line is expected to move the statistics phase by double digits with no statistics code touched — measure the `bisect-l0-probe.pl` metric, not the stats code, when that happens.


## Drop 1.6 — the access-log family (#444, 2026-09-03)

Record: `features/444-access-log-format-family-and-user-surface.md` (requirements, findings, D1-D17). What changed in the registry:

- **Seven entries replace the five access entries.** Root names `access_common` and `access_combined`; variants named by the fields they add: `access_common_duration` (Tomcat 6-9 write `%D` in milliseconds, Apache HTTP Server and Tomcat 10.1+ in microseconds, and nothing on the line tells them apart, so the entry declares milliseconds and `-du us` names the other producers; #444 D3 as revised by the architect on 2026-09-04 retired the `_ms` / `_us` unit variants, which no line could distinguish), `access_common_duration_thread_session` (the same rule), `access_common_duration_bracketed` (CodeBeamer: the first bracketed token is the duration, its unit read from the line through the `duration_from_unit_token` transform into a block-local `duration_unit_token` capture; the second token is matched and discarded), `access_combined_duration` (combined plus a trailing duration, the generic spelling of `jboss_access`, which keeps its slot ahead of it as the stricter pattern — numeric bytes, non-empty quoted fields — so the generic entry's samples avoid it by a dash for the bytes or an empty referer), `access_combined`, `access_common`. Every pattern is `$`-anchored and declares exactly its shape's fields (D4); the family is ordered longest tail first, `jboss_access` between the bracketed and the combined-duration shapes.
...
Record: `features/476-per-format-log-level-declarations.md` (requirements, findings, D1–D10 and the amendment of 2026-09-13). What changed in the registry:

- **A `levels` key on the entries that write categories of their own.** The seven access entries declare `1xx` through `5xx`, the G1 entry its six pause kinds, the Workgroup Manager entry the ten names its letter map produces, the Edge C SDK entry `TRACE, DEBUG, INFO, WARN, ERROR, FORCE, AUDIT`, the `csv` entry `DATA`, and the pin-only verification entry `INFO, WARN, ERROR, FATAL`. Every other entry declares nothing, and nothing is inferred from that.
- **The declaration is seated at detection, not at build** (`format_seat_declared_levels()`): every entry the file's evidence sample recognised seats its levels before the file's first line is gated, and the first-match bind seats the bound entry's for a file that could not be sampled or that `-lf` pinned. The set only grows during a run. The per-line category gate is unchanged: one constant-time `exists` on one global hash, with no per-format check on the accept path.
...
- `unregistered_levels: TOKEN=N,TOKEN=N,...|-` — the levels seen in this file's category position that the recognised vocabulary does not carry, each with the number of lines of this file that carried it. Each token is the raw value exactly as it reached the category gate: a level word, a folded status family outside `1xx`–`5xx`, or an unmapped Workgroup Manager msgtype letter. Order is descending by count, and by token ascending within an equal count, so the value is deterministic across runs on the same input. The literal `-` when the file produced none, which is every file in the current corpora; the key is always emitted, in both the with-detection-data and the placeholder branch of `emit_format_detection_verbose()`, so a harness asserting it can distinguish "none observed" from "the key is gone" (`tests/HARNESS-DESIGN.md` § *Harnesses must fail on missing anchors*). The format that produced the tokens is the file's own `format:` key in the same block and is not repeated here. Owning record: `features/476-per-format-log-level-declarations.md` § D7 (a new `-V` key under `format-detection`; `excluded_other` is untouched). Consumer: `tests/validate-format-detection.sh`, scenario `unregistered-levels-per-file`.
...
- `family: <name>=<members>` — one line per family (#444 D5): entries sharing a root name and sibling shapes, in static spec order. A family is distinct from a unit variant group: the group means one slot whose member evidence chooses, the family means the shapes the ambiguity note, `--help formats` and this section list together. Today: `family: access=mt3ts,mt12,mt9,mt19,mt20,mt3,mt4`.
