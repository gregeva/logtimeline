...
- **`fields: user=N/L session=N/L thread=N/L`** (#444 D8), after `probes:` in each variant group's block: for the selected member, the identity fields it declares, each as populated lines over the sampled lines the member's pattern matched (populated: defined, non-empty, not a bare `-`); `-` for a field the member does not declare. The same count earns `field_populated` (0.5, one weight table) per populated field in `candidates:`, so a column that has data ranks above the same shape whose column is empty.
...
- `entries: N` — every entry in `format_registry_specs()`, scanned, pin-only and stateful alike (23: 21 scanned + the pin-only `mtvfy` + `csv`).
- `scanned_entries: N` — entries the scan can recognise, variant members included (21 since #444 replaced the five access entries with the nine-member family). Changes only when a scanned format is added or removed — same commit updates this contract and the harness.
