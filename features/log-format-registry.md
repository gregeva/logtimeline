...
- **D53 — Evidence is gathered by a multi-spot sampling pass; the held-line window is the fallback for non-seekable input.** Before the first line of a file is processed, the detect stage reads a sample of lines from several points in the file — the start, the end, and points between — and the first decision (variant, unit) is made on that evidence. Where the input cannot be repositioned (a pipe, a compressed stream — neither read today), the detect stage falls back to the #58 two-phase-store window (D30): hold the first N lines, decide, replay. Probes consume a list of sample lines, never a file position, so the two sources are interchangeable to the decision logic. Rationale: a front-only window of any size cannot see the far end of a file, and the date-layout case is usually decided by the span between the first and last timestamps (a file spanning days under one layout spans months, runs backwards, or hits an impossible month under the other), not by a single line; one specimen's decision line (571 of 5,416 in the Integration Runtime file) says nothing about the next file's. Confidence is progressive: the sample yields the best assumption available without hard indications, and the steady-loop probes correct it when hard evidence arrives. Locked 2026-08-22.
...
1. **Sample shape.** The number of sample spots, the lines read per spot, and their placement — the start and the end of the file are mandatory, with points between — sized so the evidence is representative of the whole file. Floor: 1,000 lines per spot (architect, 2026-08-22: anything under a thousand is not worth considering). Measured across the fixture families and the D51 fixtures by what each candidate shape *sees* — recognised lines, formats present, date span under each layout, duration values — not by whether it reaches a particular line of a particular specimen.
2. **Probe cost on the sample.** Full probes (D52 a/b/c) run on every sampled line; #388 reports the one-time per-file cost of the probes over the sample and of the sampling pass's own I/O (repositioning, discarding the partial line landed in, reading), with the many-small-files case — where the pass is paid once per file — measured explicitly.
3. **Non-interference.** Output with the sampling pass on is byte-identical to output with it off, including files shorter than the sample (entirely sampled) and files whose sample spots overlap. The fallback window's side-effect-ordering parity (proven at N=1000 during #58 S4/S5) is re-proven at whatever N the fallback ships with.
...
5. **#17's unit sampling** rides the same sample (a better sample than the ~100 front lines it originally asked for).
6. **Deliverable shape:** the sampling pass is the default path for seekable input; the fallback window keeps its hidden `--detection-window` override and default N; `window_size:` telemetry stays and gains a sibling key describing the sample shape, named in #388's `-V format-detection` contract addition. Telemetry for "decision stabilised at line L" is this drop's, not #388's.
...
| same IR slice | `app.log` | no evidence → default; probe contradiction reported; decided from the sample before line 1 |
...
| #60 | 3 | 2 | Configurable metric visibility and purpose | #384 (re-cut 2026-08-22; was #58) | **In** |
