...
- **Help and usage (2026-09-16).** There is a lot of overlapping syntax here. The existing options are kept; the help does not over-advertise every option, as it would be too long. The help is intuitive and coherent.
...
- **D3 — LOCKED 2026-09-16 (architect) — One option names anything to expose; the thread gains a shorthand; the existing shorthands stay.** `-x <name>` / `--expose <name>` (`-x thread`, `-x fileName`); `-xt` / `--expose-thread` is the shorthand for `-x thread`, the way `-xs` (session), `-xu` (user) and `-xqs` (query string) remain as they are. Neither `-x` nor `--expose` collides with an existing option (the parser runs without abbreviation).
