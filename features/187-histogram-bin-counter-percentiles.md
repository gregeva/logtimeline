...
| 4 | **Deliver #34 implementation** — heatmap and histogram consume #189's primitives | **#34** | First production consumer of #189. Simpler consumer (no percentile interpolation); verifies primitives work in production before more complex consumers build on top. | No dependency from this file |
| 5 | **Deliver #187 Phase 2** — summary-table percentiles consume #189's primitives | **#187 Phase 2** | Second consumer. | **This file's primary implementation step; corresponds to R9 Phase 2** |
...
Steps 1 and 2 may proceed in parallel once the audit has produced enough consumer-side requirements for the research to evaluate bin-derived interpolation against. Step 3 (#189 implementation) cannot complete until step 2 lands the algorithm choice, but its scaffolding (primitive signatures from the consumer-side requirements) can begin earlier. Step 5 (this feature's Phase 2 implementation) cannot start until step 3 has the percentile-interpolation primitive complete; the partition / assignment / counter primitives (needed first by #34 at step 4) can land before the interpolation primitive.
