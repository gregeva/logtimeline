...
| `-hi, --hide <section>` | Hide a section of the output: `title`, `progress`, `timeline` (`tl`), `histogram` (`hg`), `options` (`opt`), `messages` (`msg`), `threadpools` (`tp`) or `summary` (`sum`); a comma-separated list, or the option repeated |
