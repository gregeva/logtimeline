...
**How ltl computes this.** Derived from already-computed percentiles: `iqr = p75 − p25`. Inherits the data model, the algorithm and the accuracy of the underlying percentiles — exact relative to observed samples in the raw values data model, or subject to within-bucket interpolation in the bin counter data model, including the extra displacement a fuzzily consolidated row (`-g`) carries. See `percentiles` for how large that is and when it matters.
...
**Why the values differ.** Nearest-rank returns a real sample; within-bucket interpolation returns a synthesised value inside the matching log-spaced bin. On the same input the two algorithms will report different `p99` (and other percentile) values, particularly in the tail. Most of that difference is the data model rather than an accuracy gap — the two are answering the question differently, not one of them answering it badly. If you switch a surface between models, expect the percentile column to move.

**How far apart they are.** A bin counter percentile is exact about *how many* observations fell where and approximate about *where inside a bucket* the target rank sits — so the reported value lands within the bucket that contains the true answer, and the finer the resolution the tighter that is. For a single message, a single time bucket or a single chart, that is the whole story, and the reported value is normally on or immediately beside the exact one. Fuzzy consolidation (`-g`) is the one case that adds to it: when several messages are folded into one row, their histograms are re-projected onto a single shared geometry, and the consolidated row's percentiles can sit up to about one bucket away from the value the pooled observations would give. Measured across every percentile of every consolidated row on real duration streams, the reading falls outside that one-bucket band about 1.5% of the time on a Tomcat access log and about 2.1% on a ThingWorx script log; on synthetic worst cases — two clusters as far apart as they can be — roughly one reading in four thousand, and never by more than a bucket and a fraction.

**What to do about it.** Nothing, for most work: a displacement of a fraction of a bucket does not change which messages are slow or which tail is heavy. It matters when you are comparing a consolidated row against an exact reference, quoting a percentile as a contractual number, or reconciling with another tool. In those cases pin the surface to the raw values data model (`--message-stats-data-model raw`), which holds every observation and returns an actually-observed value; or drop `-g`, since rows that were never consolidated do not carry the extra displacement. `min`, `max`, `mean`, `std_dev` and the distribution-shape statistics stay exact under either model, consolidated or not.
