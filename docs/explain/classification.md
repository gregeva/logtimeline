...
This is the coverage argument of the Google SRE Workbook chapter on implementing SLOs (<https://sre.google/workbook/implementing-slos/>): a service-level indicator is a ratio of good events to all events, and the denominator is only honest when the source sees all events. A reliability figure — successes over classified lines — is therefore legitimate only over an event ledger. Each format declares whether it is one, the property is reported per file on `-V format-detection` (`event_ledger`), and consumers that compute a ratio read it to decide whether to show one.
