...
- Locked unified percentile-and-histogram contract — research, industry grounding, implementation-readiness audit, prototype, and validation report shipped together; primitives are uncalled this release pending consumer migrations (#187, #195)
- New histogram bin-counter primitives + `=== PERCENTILE MODE ===` `-V` block + new CLI flags (`-pp/--percentile-precision`, `-pbpd/--percentile-buckets-per-decade`, `-ep/--exact-percentiles`) + analyst docs; baseline regression byte-identical (#189)
- New regression suites: `tests/validate-percentile-mode.sh` (locked `-V` field stability) and `tests/validate-help-layout.sh` (auto-derived column-alignment guard for `--help`) (#189)
- Short/long form parity enforced project-wide: long forms added for `-pbpd`, `-hgbpd`, `-hgb`; short forms added for `-pp`, `-ep`; previously-undocumented `-hgbpd` / `-hgb` now in `--help` and `docs/usage.md` (#189)
...
The #189 primitives ship uncalled — no consumer is migrated, so baseline regression remains byte-identical. Consumer migrations (#34 next) are each follow-up tickets per `features/187-...-percentiles.md § R9` phasing.
