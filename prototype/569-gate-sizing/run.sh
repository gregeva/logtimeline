...
#   usage: run.sh <batch dir> <output dir> [sources=500] [timed passes=3] [probe=probe.pl]
set -euo pipefail
here="$(cd "$(dirname "$0")" && pwd)"
batches="$1"; out="$2"; n_src="${3:-500}"; passes="${4:-3}"; probe="${5:-probe.pl}"
mkdir -p "$out"
...
for b in day-dl/batch-plain_200 app/batch-plain_ERROR day-mixed/batch-plain_200 \
    ...
    perl "$here/$probe" "$out/subs.pl" "$label" "$batches/$b.txt" "$n_src" "$passes" \
