...
- 2026-08-24: CRITICAL — THE COMPLETE HARNESS SUITE PLUS A BENCHMARK CASE IS THE COMPLETION GATE FOR EVERY ISSUE, BEFORE THE MERGE. An issue is not done until every `tests/validate-*.sh` exits 0 **and** `./tests/baseline/run-benchmark.sh single-day-access-log-standard --label <name>` compared against the last released baseline shows no metric worse by more than 5% (the threshold is deliberately tight to catch regressions early; a breach is stop-and-investigate, since one run is not median-of-3). Both run BEFORE the PR is opened, not at release-cut time. #413 was merged on 4 of 22 harnesses and no benchmark; the suite was then run afterwards and passed, which is luck about the outcome, not a defence of the process. The mechanical gate before `gh pr create`: have I run all 22 harnesses and the benchmark case on THIS commit? A run that predates the last amend does not count.
...
1. **COMPLETION GATE — the work is not done until both pass, on the commit being merged.** Neither is a release-cutting task and neither waits to be asked for:
   ...
   b. **A benchmark regression check**, compared against the most recent released baseline TSV in `tests/baseline/results/`:
   ```bash
   ./tests/baseline/run-benchmark.sh single-day-access-log-standard --label {issue}-gate
   ./tests/baseline/compare-results.sh summary tests/baseline/results/v{last-release}.tsv tests/baseline/results/{issue}-gate.tsv
   ```
   **Any metric worse by more than 5% is a stop-and-investigate**, not an automatic block — a single run is not a median-of-3, so confirm before concluding. Delete the gate TSV afterwards; it is not a deliverable. The threshold is deliberately tight to catch regressions early rather than at release-cut time.
   ...
   Re-run the gate after any amend or added commit: **a gate run that predates the commit being merged does not count.** Narrowing to "the harnesses this change touches" is exactly how #413 came to be merged on 4 of 22 harnesses with no benchmark (2026-08-24).
...
8. Add a completion comment to the GitHub issue: commit hash, branch name, PR #, merge commit, summary of what shipped.
