...
- 2026-08-30: CRITICAL — TEST CASES ARE DERIVED FROM THE REQUIREMENT AND WRITTEN BEFORE THE CODE; A TEST WRITTEN AFTERWARDS CONFIRMS THE CODE, INCLUDING ITS MISREADING OF THE REQUIREMENT. Across #451, #448 and #446, seven defects reached the architect from real runs. All seven were in visual output — colour, fill shade, bar length, line shape, label alignment — and every one passed the harnesses, with assertions genuinely running. `tests/validate-summary-contribution-bar.sh` passed 23 assertions against five simultaneous #448 defects, because the assertions had been written from the implementation: the bar was built from a newly invented `-HL` colour vocabulary instead of the timeline bar mechanism the requirement named four or five times, and the tests were then written to prove *that* worked. Testing cannot catch a substituted design when the tests are derived from the substitution. The corrective is ordering, not more tests: **requirements produce acceptance criteria; the criteria are recorded in the feature doc and agreed before implementation; implementation is done when they pass.** Each criterion is triaged assertable / unassertable / **unknown**, and the third state is the one that gets skipped — an unresolved "how would I check this?" does not disappear, it is answered later by whatever the already-written code permits. It therefore becomes prototyping scope: `prototype/README.md` trigger (d), whose exit is a demonstrated assertion method or a recorded decision that the requirement cannot be asserted. Where a requirement names an existing mechanism to reuse, the criterion names it too, so it cannot pass against a substitute. Two supporting rules from the same incident: a visual surface is verified by looking at the rendered output on real data, never by grepping escape sequences (an escape-code assertion passes against a fill from the wrong colour table, two rows that render identically when they must differ, and a bar wider than the data supports); and the verification method is proposed **with its cost**, the architect deciding whether it is worth paying, since some requirements are obvious to any tester on a single run and others need real machinery. Ten feature docs already carried an `## Acceptance criteria` section; the three written this cycle dropped it. Full rule: `docs/test-driven-development.md`; prototyping's part: `prototype/README.md`.
...
- 2026-09-02: CRITICAL — A PERFORMANCE GATE IS RUN ON A CASE THAT CONTAINS THE CHANGE, AND ITS FIGURES ARE REPORTED, NEVER ITS VERDICT. Two failures on #520 (an inert `-so` under `-n 0` still holding the bytes aggregate-family capture alive), sharing one root: a threshold was substituted for the measurement. **(a)** The step 1(b) before/after was reported to the architect as "nothing worse by more than 5%" with not one number shown — converting a stop-and-investigate trigger into a pass verdict, and waving away the two rows that DID cross 5% by quoting `compare-results.sh`'s own "below noise floor" label instead of investigating them. The architect: "you can't just take such a judgment, especially when we are investigating performance regressions blindly on some threshold". The measured headline was in fact an improvement (`TIMING/total` −143 ms, which the tool itself labelled IMPROVE), mis-presented as a tolerated regression. **(b)** The case run was `single-day-access-log-standard`, which passes no options — so the demand flag the change moves resolves to 0 on both sides and the run never enters the gated path; its unchanged `MEMORY/log_analysis` said so plainly and went unread. Every delta in that table was run-to-run variation from a single run per arm. The attributed measurement, once the architect forced it — `-n 0 -so bytes_min` on the 761,698-line access log, five interleaved runs per arm against the base commit's `ltl` — gave `parse/read_files` 7.399 → 7.302 s (−97 ms, −1.31%) with non-overlapping ranges. Compounding both, the defence offered for the mismatch was that the change "doesn't affect the hot path": gating a per-line capture IS a hot-path change, and denying it denies the fix its own purpose. Two mechanical gates. Before launching step 1(b): name the option state the change alters and confirm the benchmark case carries it — if it does not, the case proves only that nothing else broke, and a targeted case that does carry it is run alongside and reported as the attributing one. When reporting any measurement: the figures go in the message — medians with ranges, before and after — and a threshold is quoted only after them, never in place of them. This is § Core Principles 9 (lead with the verdict and the evidence follows) and 12 (findings before disposition) applied to a number the architect never got to see.
...
1a. **Acceptance criteria — MANDATORY, part of planning, never after the code.** The feature doc gains an `## Acceptance criteria` section derived from the requirements: each criterion a condition and an observable outcome, covering the key requirements — the ones whose failure would mean the feature was not delivered. Each is triaged **assertable** (method known), **unassertable** (recorded as a gap, architect decides whether the requirement stands), or **unknown** (the method is not yet known — this becomes prototyping scope, trigger (d) below). Where a requirement names an existing mechanism to reuse, the criterion names it too, so it cannot pass against a newly invented substitute. Implementation is done when the criteria pass; a criterion that turns out to be unbuildable mid-implementation is a stop-and-raise, because it means the requirement and the design have diverged. Full rule: [`docs/test-driven-development.md`](docs/test-driven-development.md). **Tests derived from the code confirm the code, including its misreading of the requirement — which is how a feature ships with a green suite and the wrong behaviour (#448, 2026-08-29: 23 passing assertions against five simultaneous defects).**
2. **Prototyping** (non-trivial features): Validate approach in `prototype/` directory first — see [`prototype/README.md`](prototype/README.md). **MANDATORY — not a judgment call — when the work introduces (a) a new or changed data model, (b) a new per-line hot-path cost, (c) any feature/fix deemed impactful by its cost profile — execution frequency × per-execution cost (a cheap operation run tens of millions of times, or an expensive one run per key at high cardinality) — or (d) a key requirement whose verification method is not yet known (step 1a's "unknown" state).** For (a)–(c) the workflow is research → prototype → validate → refine design → record decisions (as Dxx in the owning feature doc) → implement. Research precedes and grounds the prototype (candidate representations, applicable measured constants, prior findings); the prototype compares implementation candidates at staged scale (1k → 10k → 100k → millions) against the current code as baseline; exit requires measured justification (medians with ranges) and lessons learned recorded BEFORE implementation begins. For (d) the prototype exits with a demonstrated assertion method — shown to distinguish known-good from known-bad input — or a recorded decision that the requirement cannot be asserted and what is done instead; where real specimens of past defects exist, the method is validated against them. The verification method is proposed **with its cost**, and where that cost is non-trivial the architect decides whether it is worth paying: some requirements are obvious to any tester on a single run and do not justify a suite. Precedent: #187 Decision 10; applied to #58 (registry data model) and #372 (pair-correlation store + sub-determination) on 2026-07-16. Data models are far more expensive to change after they ship than to validate up front.
3. **Scheduling**: Get explicit user approval before implementation
4. **Implementation**: Update feature doc progress as you work
5. **Testing**: Use sample files in `logs/` directory (see `docs/test-logs.md`). **A change to a visual surface is verified by looking at the rendered output on real data, not by grepping for escape sequences** — an escape-code assertion passes against a fill drawn from the wrong colour table, two rows that render identically when they must differ, and a bar wider than the data supports.
