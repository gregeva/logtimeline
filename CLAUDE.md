...
- 2026-05-26: CRITICAL - For testing, debugging, validation, and any inspection of a long-running command's output: run the command ONCE, redirect stdout+stderr to a temp file, then `grep`/`head`/`tail`/`sed`/`diff` against that file as many times as needed. NEVER re-run the same command just to get a different slice of its output. Especially forbidden: re-running `validate-statistics.sh` (~3 min), `validate-doc-examples.sh` (~40s), or `validate-histogram-bin-counters.sh` (~2 min) to see "what the next line was" or "what the failure looked like" — that information is in the output you already captured. Violated repeatedly during #287's L3 oracle work: ran `validate-statistics.sh` ~8 times in a single debug cycle, each iteration fixing one Python error, costing ~25 minutes of the user's wall-clock time for ~80 seconds of fix work. The correct loop for any harness-failing-on-something investigation is: (1) capture full output to `/tmp/<harness>.out` once; (2) inspect via grep against the file; (3) if a subprocess (oracle, helper, child script) is failing, invoke that subprocess DIRECTLY on a minimal input — milliseconds per iteration; (4) use `--scenario <name>` or equivalent single-test selection flags for the harness when iterating; (5) only re-run the full harness as the final go/no-go gate. Long-running harnesses are NOT debuggers.
