...
- 2026-07-09: SIBLING OPTIONS SHARE ONE PARSER. Options that accept the same operand vocabulary (e.g. `-hm`/`-hg` metric names) must resolve operands through one shared code path, or carry an explicitly stated matching contract in the feature doc that both implementations are tested against; duplicated per-option parsing is how case/alias semantics silently diverge (#327). When adding an option that accepts an existing option's operands, route it through the existing resolution code.
